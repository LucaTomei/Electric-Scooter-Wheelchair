...
class Firmware_Utils(object):
	...
	def PatchImm(self, data, ofs, size, imm, signature):
	...
	def FindPattern(self, data, signature, mask=None, start=None, maxit=None):
...
class FirmwarePatcher():
    ...
    # To disable Ninebot/Xiaomi automatic DRV updates on firmware changes
    def version_spoofing(self):
        sig = [0x4F, 0xF4, 0x93, 0x70, 0xA0, 0x86, 0x12, 0x48, 0x00, 0x78]
        ofs = self.Firmware_Utils_OBJ.FindPattern(self.data, sig)
        pre = self.data[ofs:ofs+4]
        post = bytes(self.ks.asm('MOVW   R0, #0x526')[0])
        self.data[ofs:ofs+4] = post
        return [(ofs, pre, post)]
    
    # To use bms directly connected to your computer
    def bms_uart_76800(self):
        ...
        while True:
            ...
            ofs = self.Firmware_Utils_OBJ.FindPattern(self.data, sig) + 2

            # USART3 address
            sig = [0x00, 0x48, 0x00, 0x40]
            try:
                self.Firmware_Utils_OBJ.FindPattern(self.data, sig)
                break
    ...
    def kers_min_speed(self, kmh):
        ...
        ofs = self.Firmware_Utils_OBJ.FindPattern(self.data, sig) + 2
        pre, post = self.Firmware_Utils_OBJ.PatchImm(self.data, ofs, 4, val, MOVW_T3_IMM)
        return [(ofs, pre, post)]
    ...
    def motor_start_speed(self, kmh):
        ...
        ofs = self.Firmware_Utils_OBJ.FindPattern(self.data, sig) + 9
        pre = self.data[ofs:ofs + 4]
    ...
    def alt_throttle_alg(self):
        ...
        ofs = self.Firmware_Utils_OBJ.FindPattern(self.data, sig) + 4
    ...
    def speed_params(self, normal_kmh, normal_phase, normal_battery):
        ...
        ofs += 10
        pre, post = self.Firmware_Utils_OBJ.PatchImm(self.data, ofs, 2, struct.pack('<B', normal_kmh), MOVS_T1_IMM)
        ret.append([ofs, pre, post])
        self.data[ofs:ofs+2] = post
        ret.append([ofs, pre, post])
    ...
    def motor_power_constant(self, val):
        ...
        ofs = self.Firmware_Utils_OBJ.FindPattern(self.data, sig) + 12
        pre, post = self.Firmware_Utils_OBJ.PatchImm(self.data, ofs, 4, val, MOVW_T3_IMM)
        ret.append((ofs, pre, post))
        ofs += 4
        ...
        ofs = self.Firmware_Utils_OBJ.FindPattern(self.data, sig, None, ofs, 100) + 2
        pre, post = self.Firmware_Utils_OBJ.PatchImm(self.data, ofs, 4, val, MOVW_T3_IMM)
        ret.append((ofs, pre, post))
        ofs += 4
    ...
    def cc_delay(self, delay):
        ...
        ofs = self.Firmware_Utils_OBJ.FindPattern(self.data, sig, mask) + 7
        pre = self.data[ofs:ofs+4]
    ...
    def max_speed(self, kmh):
        ...
        ofs = self.Firmware_Utils_OBJ.FindPattern(self.data, sig) + 4
        pre, post = self.Firmware_Utils_OBJ.PatchImm(self.data, ofs, 2, val, MOVS_T1_IMM)
        ret.append((ofs, pre, post))
